...
const AppDataSource = new DataSource({
  ...
  host: 'localhost',
  port: 5432,
  username: 'karyasiddhi_user',
  password: 'karyasiddhi_pass_2025',
...
async function seed() {
  ...
  await AppDataSource.query(`
    INSERT INTO departments (id, name, code, ministry, "headOfDepartment", "employeeCount", "createdAt", "updatedAt") 
    VALUES 
      ('650e8400-e29b-41d4-a716-446655440001', 'Digital Infrastructure', 'DI-001', 'Ministry of Electronics and IT', 'Arun Singh', 4, NOW(), NOW()),
      ('650e8400-e29b-41d4-a716-446655440002', 'Citizen Services', 'CS-002', 'Ministry of Electronics and IT', NULL, 4, NOW(), NOW()),
      ('650e8400-e29b-41d4-a716-446655440003', 'Data Governance', 'DG-003', 'Ministry of Electronics and IT', NULL, 5, NOW(), NOW())
  ...
  // Verify
  const goalCount = await AppDataSource.query('SELECT COUNT(*) FROM goals');
  const kpiCount = await AppDataSource.query('SELECT COUNT(*) FROM kpis');
  const deptCount = await AppDataSource.query('SELECT COUNT(*) FROM departments');
  
  console.log(`📊 Created ${deptCount[0].count} departments`);
  console.log(`🎯 Created ${goalCount[0].count} goals`);
  console.log(`📈 Created ${kpiCount[0].count} KPIs`);
