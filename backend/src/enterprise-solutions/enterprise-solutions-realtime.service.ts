...
export class EnterpriseService {
  ...
  async getCarnivalOfProductivityData(userId?: string) {
    try {
      const goals = await this.getUserGoalsStats(userId);
      const kpis = await this.getUserKPIStats(userId);
      const issues = await this.getUserIssuesStats(userId);
  ...
  async getZeroKnowledgeGovernanceData(userId?: string) {
    try {
      const goals = await this.getUserGoalsStats(userId);
  ...
  async getEnhancedGamificationData(userId?: string) {
    try {
      const achievements = await this.getUserAchievements(userId);
      const goals = await this.getUserGoalsStats(userId);
  ...
  async getTidalWaveAnalyticsData(userId?: string) {
    try {
      const goals = await this.getUserGoalsStats(userId);
      const issues = await this.getUserIssuesStats(userId);
      const activity = await this.getRecentActivity(userId, 30);
